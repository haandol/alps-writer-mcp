...
**Document profiles** (`src/profiles.ts`) — Full ALPS and four-section Lite ALPS metadata: section titles, dependency graph, authoring order, optional Sections, file suffix, template/guide paths, and dynamic Feature sections where applicable. Section ranges are derived from each profile. Lite reuses Full's conversation-led authoring behavior while keeping independent document state and completion. Lite uses `Overview → Solution and Acceptance Tests → Out of Scope → Demo Scenario`; Section 3 is optional and omitted from Markdown when unwritten. Section 2 records every Required Acceptance Test with an observable pass condition, and Section 4 automatically generates and shows a Demo Scenario that covers every test.

**Conversation-led authoring** — Full ALPS retains its pre-Lite section-by-section conversation: explain the section, ask one focused question or at most two closely related questions when context is missing, integrate the answer, present a contract-complete approval digest, and save only after confirmation. Lite ALPS reuses that interaction while reducing the template to four product/PoC Sections; optional Section 3 skips the question when no explicit exclusion exists and the approved boundary is not materially ambiguous. Shared Lite support must not rewrite Full skills, guides, examples, or wizards.
