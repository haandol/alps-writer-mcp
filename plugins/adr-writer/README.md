...
`adr-structure-lint` handles the **deterministic half** of the reviewer's R-rules (enum/format, presence, counts, cross-reference consistency); reviewer subagents focus on the judgment half. `/adr-new` carries that judgment half itself rather than delegating — it authors under the same R1-R20 rules, so `adr-reviewer` runs only via `/adr-review`, on ADRs edited by hand or inherited, where no authoring context survives. `/adr-impl-refactor` uses an isolated read-only reviewer to classify efficiency and reuse improvements, while the main session applies only candidates that pass the behavior-preserving safety gate and before/after tests. If no isolated reviewer exists, every item remains proposal-only; when no patch is kept, the passing targeted baseline is reused. `/adr-impl-review` is the pre-promotion completion gate and records elapsed time, per-perspective finding counts, unverified risks, and executed test commands before `PASS` permits `Accepted`. Claude Code can use the named definitions under `agents/`; Codex reads the same definitions and passes them to generic subagents. `/adr-new`, `/adr-impl`, and `/adr-sync` each call the harness at their verification step — see their SKILL.md.
