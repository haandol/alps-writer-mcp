...
| `/adr-impl-review [category]`    | Derive the complete implementation scope from every ADR decision and contract row instead of treating the diff as a ceiling, then select `standard` or `full` by risk. Both explain ADR intent first, use subject-specific narrative sections ordered by importance, show findings before collapsible evidence, and open a validated standalone HTML report with a table of contents. The ordinary main-session response never prints or grades the collapsed comprehension questions; interactive grading starts only by explicit user request (report-only) |
...
| `scripts/adr-impl-review-report.mjs`      | validated `findings.json`                              | Renders the common standard/full Evidence Package as one self-contained `adr-impl-review-report.html` with a table of contents, Markdown and compact Mermaid rendering, finding-to-contract anchors, state-based evidence disclosure, optional self-check, and ruling controls only for human-decision findings.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
`adr-structure-lint` handles the deterministic half of the reviewer's R-rules. `/adr-impl-review` is the pre-promotion completion gate and an on-demand audit of an existing implementation. Every invocation derives the complete implementation scope from the ADR, traces direct and indirect call paths plus related tests, and keeps the diff as separate change context. Standard mode uses a decision ledger, a sufficiency perspective, concise notable implementation choices, and targeted tests for localized implementations; full mode adds separately grounded necessity and sufficiency perspectives. Both modes reject missing or source-referencing language-standard function documentation and happy-path-only test coverage, render the validated Evidence Package to `adr-impl-review-report.html`, and run `adr-impl-review-open.mjs` to open it with the host's default application. The report starts with the outcome and ADR intent, then follows one or more subject-specific sections and shows findings before detailed contract evidence. Proven coverage, scope, metrics, implementation choices, and comprehension are collapsed by default; exceptional coverage stays open. The local self-check can reveal criteria only after answer entry, while semantic grading remains an explicit later request and never changes the implementation verdict or ADR Status.
