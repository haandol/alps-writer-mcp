...
| `/adr-impl-review [category]`    | Select `standard` or `full` by risk. Both start with a junior-readable verdict/impact/action/risk summary and preserve read-only evidence; full adds necessity and sufficiency perspectives while the model chooses the agent topology (report-only)                                                                                                                                                                      |
...
| `scripts/adr-impl-review-validate.mjs` | `/adr-impl-review` artifact directory                  | Validates the junior-readable At a glance handoff, decision ledger, concise notable implementation choices, the common review report, evidence-complete `findings.json`, and repair guidance when the verdict requires it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
`adr-structure-lint` handles the deterministic half of the reviewer's R-rules. `/adr-impl-review` is the pre-promotion completion gate: standard mode uses a decision ledger, a sufficiency perspective, concise notable implementation choices, and targeted tests for localized changes; full mode adds separately grounded necessity and sufficiency perspectives. The model chooses whether those perspectives run through named agents, generic subagents, or main-session passes. Both modes start with verdict, impact, action, and risk. They include the smallest grounded Mermaid when the review spans multiple participants, states, boundaries, failure paths, or data relationships; a local one-file PASS may omit it. Repair guidance appears only when a finding requires it, and the optional HTML keeps implementation choices read-only while findings remain reviewable.
