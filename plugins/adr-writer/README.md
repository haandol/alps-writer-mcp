...
ADR-driven development cycle for Codex and Claude Code. The ADR admission gate records only durable requirements and architectural decisions, leaving replaceable libraries, SDKs, frameworks, and credential/auth wiring in code. User-facing authoring leads with a Decision Digest, records one reviewable obligation and implementation-independent observable evidence per contract row, and keeps implementation defaults below ADR resolution. Edits and sync lead with semantic changes. Human-facing review reports start with verdict, impact, action, and risk in junior-readable language, then preserve requirement-by-requirement evidence. Grounded Mermaid diagrams externalize multi-participant, state, dependency, and failure flows without becoming a format quota. The complete ADR remains authoritative for decisions and contracts; review summaries remain disposable views over code.
...
| `/adr-impl-review [category]`    | Select `standard` or `full` by risk. Both use `Background → Intuition → Code walkthrough → Comprehension check`, preserve read-only evidence, and keep each section's internal form flexible; full adds necessity and sufficiency perspectives while the model chooses the agent topology (report-only)                                                                                                                                                                                                                                                    |
...
| `scripts/adr-impl-review-validate.mjs` | `/adr-impl-review` artifact directory                  | Validates the junior-readable At a glance handoff, the fixed explanation/check section order, one-to-five hidden-answer comprehension questions, decision ledger, concise notable implementation choices, the common review report, evidence-complete `findings.json`, and repair guidance when the verdict requires it.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
`adr-structure-lint` handles the deterministic half of the reviewer's R-rules. `/adr-impl-review` is the pre-promotion completion gate: standard mode uses a decision ledger, a sufficiency perspective, concise notable implementation choices, and targeted tests for localized changes; full mode adds separately grounded necessity and sufficiency perspectives. Both modes reject missing or source-referencing language-standard function documentation and happy-path-only test coverage. The model chooses whether those perspectives run through named agents, generic subagents, or main-session passes. Both modes start with verdict, impact, action, and risk, then use fixed `Background`, `Intuition`, `Code walkthrough`, and `Comprehension check` headings; only the headings and order are fixed. The last section contains one to five free-response questions whose grading criteria stay hidden until the user answers. A code `PASS` still completes the implementation lifecycle, but the report advises against opening or sending the PR until every question passes. Complex relationships receive the smallest grounded Mermaid; a local one-file PASS may omit it.
