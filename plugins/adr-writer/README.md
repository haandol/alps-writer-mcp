...
| Command                          | Role                                                                                                                                                                                                                                                                       |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/adr-new <category>`            | Apply the admission gate, then author a durable architectural decision directly; material decision premises are explicit, while implementation-only choices create no ADR                                                                                                  |
...
| `/adr-impl-review [category]`    | Select `standard` or `full` by risk. Both expose exact code-level choices in an Implementation Choice Ledger; full adds necessity review, grounded Mermaid repair artifacts, and interactive rulings (report-only)                                                         |
...
| `scripts/adr-impl-review-validate.mjs` | `/adr-impl-review` artifact directory                  | Validates the decision ledger, Implementation Choice Ledger, concise standard report, or full junior repair guide and evidence-complete `findings.json`; only full mode proceeds to interactive HTML.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
`adr-structure-lint` handles the deterministic half of the reviewer's R-rules. `/adr-impl-review` is the pre-promotion completion gate: standard mode uses a decision ledger, an Implementation Choice Ledger, isolated sufficiency review, and targeted tests for localized changes; full mode adds independent necessity/sufficiency reviews, grounded Mermaid explanations, and an interactive page for ruling on findings and AI-selected defaults. Intent and regeneration completeness are approved before implementation, so neither mode repeats a routine human gate afterward. Both record the selected mode, exact implementation choices, elapsed time, findings, unverified risks, and executed test commands before `PASS` permits `Accepted`.
