...
const SOURCE = `
현재 4-Section Lite ALPS에서 Sections 1-3은 승인되어 저장되었다.
...
export default {
  ...
  description:
    "Current Lite ALPS must skip optional Section 4 when no explicit exclusions exist and must not route completion into Full ALPS.",
  ...
  build(dir) {
    ...
    return [
      ...
      `In the tail use SKIP_SECTION_4, NO_INVENTED_EXCLUSIONS, LITE_INDEPENDENT, and EXPORT_READY once each.`,
      TAIL_SPEC,
    ].join("\n");
  },
  ...
  score({ tail, output }) {
    ...
    return [
      expectText(tail.raw, /SKIP_SECTION_4/i, "skips optional Section 4"),
      expectText(
        tail.raw,
      ...
      expectText(tail.raw, /EXPORT_READY/i, "continues to Lite export"),
      expectNoText(
        visible,
