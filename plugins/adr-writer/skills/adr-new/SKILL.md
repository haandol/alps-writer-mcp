...
3. **What choice are you making? One core line.** (Decision)
   ...
   - **Expose material decision premises.** Ask once: "What facts are we assuming while choosing this architecture, what supports each assumption, and what would we reconsider if it were false?" Route each answer before writing it. A value or rule the result must honor goes in the requirement contract. A fact that changes which architectural alternative is preferred goes in optional `Decision premises` with basis, confidence, and reconsideration impact. A replaceable library, SDK, adapter, internal structure, timeout, pool size, retry count, or other implementation default stays out of the ADR and is surfaced later by implementation review. If an unverified premise changes the contract or a durable architecture boundary, resolve it before approval rather than recording it as accepted fact.
...
- Body structure: Status / Context / (optional) Decision premises / Decision Drivers / Decision / alternatives / Consequences / (optional) Implementation Notes / Related. **The four required sections are Status, Context, Decision, and Consequences**, and `adr-structure-lint` hard-checks their presence. Decision premises appear only when they materially discriminate the choice. Decision Drivers and the alternatives section are strongly recommended (a warning when absent), and Implementation Notes is an optional section kept only when there are architecture-level implementation considerations (matching README's `## ADR template`).
...
- **Keep Decision premises separate from both contracts and code** — include only premises that could change the adopted alternative. Give each a basis, `high|medium|low` confidence, and the decision impact if false. Never move requirement values into this section, and never persist replaceable implementation defaults there. Omit the section when empty.
...
**Decision premises**: <premise → basis → confidence → what changes if false; omit when none>
