...
- **`dependsOn`** — 2단계 5번에서 사용자가 선행 조건으로 지목한 카테고리 키들을 배열로 기록한다. `/adr-impl` 의 선행 게이트가 읽는 바로 그 필드이며 같은 카테고리-키 id-space 다. 기존 카테고리 키만 참조하고 비순환(자기 자신 포함 금지)을 유지한다 (스키마: `mapping.schema.json` 의 `dependsOn`). 사용자가 5번에 "없음" 으로 답했다면 `dependsOn` 을 `[]` 로 기록한다 — 빈 배열은 "의존 없음을 명시적으로 점검 완료" 의 뜻이고, `/adr-impl` 이 안내 없이 진행한다. `dependsOn` 을 아예 생략하면 `/adr-impl` 이 "의존 미선언" 으로 보고 한 줄 경고를 띄우므로, 5번을 물어본 `/adr-new` 경로에서는 생략하지 않는다.
