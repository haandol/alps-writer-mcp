...
1. **코드 정합(code-first)**: 통합본의 내용은 "옛 ADR들이 뭐라고 적었나"가 아니라 **현재 코드가 실제로 무엇을 하는가**를 기준으로 쓴다. 코드가 source of truth.
...
- `docs/adr/.mapping.json` 읽기 — 카테고리 → `codePaths` 매핑. 코드 정합 검증에 필수다. 없으면 `adr-sync`의 "매핑 파일이 없을 때" fallback을 먼저 수행해 매핑을 만든 뒤 진행한다 (추측 금지).
...
2. 그 카테고리의 `codePaths`를 grep/read해 **현재 코드가 실제로 무엇을 하는지** 확인한다. 옛 ADR이 묘사한 동작 중 코드에 더 이상 없는 것, 코드엔 있는데 어느 ADR에도 없는 결정, 값이 바뀐 것(임계값·상태값·연동 방식)을 식별한다.

통합본은 이 코드 사실을 기준으로 쓴다 — 옛 ADR은 "무엇이 결정 대상이었나"를 알려주는 입력일 뿐, 최종 진술의 근거는 코드다.
...
2. 각 주장을 해당 카테고리 `codePaths`에서 grep으로 검증.
3. **불일치 발견 시**: 코드가 source of truth이므로 통합 ADR을 코드에 맞게 수정한다.
...
- 코드가 source of truth — 통합본이 코드와 충돌하면 통합본을 고친다 (정책상 ADR이 옳아야 한다는 사용자 명시가 없는 한).
