...
3. **Build the plan**
   ...
   - Only when the user asks to split, offer up to three candidates. Split into separate ADRs only for independent decisions. Keep one inherently difficult decision in one ADR and offer implementation steps instead; never split by technical layer.
...
4. **Implement**
   ...
   - **Make material implementation discretion reviewable** — while implementing, keep an ephemeral list of choices that a reviewer would otherwise have to rediscover: the topic, exact selected value or behavior (including units), why it was selected, the code evidence, plausible alternatives, and what changes if the choice changes. Include dependencies, tuning defaults, internal fallback behavior, and inherited project conventions only when they affect operation, maintenance, cost, or failure behavior; do not list every identifier or local expression. This list is input to `/adr-impl-review`'s Implementation Choice Ledger and is never written into the ADR.
