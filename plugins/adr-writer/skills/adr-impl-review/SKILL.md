...
- `Undecided behavior` → the user decides whether to add it to the ADR as a decision or remove it from the code. Adding it is an ADR edit, so it goes through the same owners — `/adr-impl` or `/adr-sync` for an in-place addition to an existing ADR, `/adr-new` when the behavior is really a separate decision deserving its own ADR.
