...
- **Do not hide material implementation discretion either.** Put code-level choices that remain below the admission gate into the ephemeral Implementation Choice Ledger with their exact selected value or behavior, basis, code evidence, change impact, confidence, and alternatives. When the actual value or basis cannot be established, use `Unverified risk`; never fill the ledger with a plausible guess.
...
Before any dispatch, read `${CLAUDE_PLUGIN_ROOT}/references/subagent-dispatch.md` completely and apply its provider capability gate and dispatch chain. Each review step below names the agent and `${CLAUDE_PLUGIN_ROOT}/agents/*.md` fallback file to use. If subagents are unavailable, use the selected mode's role-specific main-session fallback and record the isolation limitation.
...
1. Build a decision ledger containing every ADR decision and requirement-contract row relevant to the diff, and an Implementation Choice Ledger containing material code-level choices absent from the ADR.
...
5. Write a concise `implementation-review.md` with these headings: `Review mode`, `Scope`, `Decision ledger`, `Implementation choices and assumptions`, `Findings`, `Tests`, and `Review limits`. Explain each choice in plain language and preserve exact values and units. Write `findings.json` with `"reviewMode": "standard"`, `necessityFindingCount: 0`, the structured `implementationChoices` array, and the normal evidence fields.
...
- the ADR's Decision premises, including their basis, confidence, and reconsideration impact
...
7. Normalize the Implementation Choice Ledger independently from findings. Every row must have a concrete selected value or behavior, basis, code evidence, impact if changed, confidence, and alternatives. A row that changes a requirement contract or durable boundary is removed from the ledger and raised as `Undecided behavior`.
...
Give the report-writer the original ADR and diff, `review-baseline.md`, all three agents' artifacts, the normalized Implementation Choice Ledger, and the verified findings and test results. Save the result as `implementation-review.md`.
...
Use progressive disclosure. Start with the one-sentence result and user impact, then show the grounded system/change flow, then provide choices, findings, code evidence, tests, and repair detail. Use plain language that a junior developer can follow, explaining each term on first use.

Include Mermaid diagrams when they reduce the amount of flow or state a reader must reconstruct, using only relationships confirmed in the actual code.
...
Before findings, include `Implementation choices and assumptions`. For each row state:

1. the choice topic
2. the exact selected value or behavior, including units
3. why it was selected and whether the basis is a project convention, existing behavior, or implementation default
4. the code evidence
5. what changes operationally or structurally if the choice changes
6. confidence and realistic alternatives

Explain that these rows are below ADR resolution and do not amend the ADR. If a row would alter the ADR contract or durable boundary, it must be an `Undecided behavior` finding instead.
...
In full mode, open the report and return the verdict, necessity/sufficiency finding counts, tests executed, elapsed time, unverified-risk count, normalized Implementation Choice Ledger, and normalized findings to the caller. In standard mode, return the concise choices and findings with the same routing metadata. A pre-promotion invocation by `/adr-impl` must not ask the user to rule `apply / skip / defer` on ordinary evidence-backed repairs; the caller owns remediation. A standalone review remains report-only and presents findings plus optional interactive rulings. For implementation choices, the available rulings are `accept`, `request change`, or `investigate`.
