...
function main() {
  ...
  const directive = [
    "[ADR-first directive] Judge for yourself whether this user request adds a new feature or changes the behavior of an existing one. A bug fix is exempt only when it restores behavior that the current ADR already decided. A fix that changes a requirement value, allowed set, state transition, permission, visibility, mandatory field, adopted algorithm, or fallback is a behavior/decision change and follows the cycle below. Lint/formatting, documentation edits, ops/deploy commands, and information lookups are exempt.",
