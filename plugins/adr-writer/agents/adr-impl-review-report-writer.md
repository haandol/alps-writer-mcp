...
- **Contract compliance** exists as its own axis because the ADR and the code are the same system at two resolutions — the ADR records the contract, the code enforces it — so a fix at the wrong level looks like a fix and is not one. When a finding sits on this axis, say which level must change: a differing value, allowed set, or transition rule means **the code** changes (the ADR owns the contract), while a differing name, signature, or wire representation means **the ADR** changes via `/adr-sync` (the code owns implementation facts). Never write a fix step that edits the ADR's value to match the code — that rewrites the requirement instead of honoring it.
