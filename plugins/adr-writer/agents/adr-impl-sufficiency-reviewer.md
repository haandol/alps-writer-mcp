...
4. **Build the separate Implementation Choice Ledger from code outward** — enumerate material behavior and values the code selected but the ADR does not specify. Include dependencies, tuning defaults, internal fallback behavior, and inherited conventions only when they affect runtime behavior, failure handling, operations, cost, or future maintenance. Apply the admission gate to every candidate: an admitted contract or durable-boundary choice becomes `[Undecided behavior]`; replaceable implementation discretion stays in the choice ledger; a material value or basis you cannot confirm becomes `[Unverified risk]`. Do not list every identifier or local expression.

The decision ledger and Implementation Choice Ledger are separate bases of the final report. D1 and D2 fill the decision ledger; the code-outward pass above makes AI-selected implementation defaults reviewable without promoting them into the ADR.
...
- **Implementation Choice Ledger** — code does something the ADR correctly leaves to implementation discretion. Record each material choice with the exact selected value or behavior, basis, `file:line` evidence, impact if changed, confidence, and alternatives. This is not a finding and does not change the verdict. If the choice passes the admission gate, classify it as `[Undecided behavior]` instead. If the actual value, call path, or basis cannot be confirmed and the uncertainty is material, classify it as `[Unverified risk]` instead of guessing.
