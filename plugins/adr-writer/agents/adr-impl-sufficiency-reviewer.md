...
- **[Undecided behavior]** — the code does **something extra the ADR never decided** (scope creep — it did not break a decision, it exceeded the decision surface). This is an ADR-first violation, so left alone it accumulates undecided features. Present two branches to the caller: if the behavior was intended, add it to the ADR as a decision — an in-place addition to the existing ADR belongs to `/adr-impl` or `/adr-sync`, while a genuinely separate decision gets its own ADR via `/adr-new`; otherwise remove it from the code. This is the inverse of `[Spec violation]` (should have done it but did not) — here the code did something it need not have.
