...
4. **Normalize each decision-ledger row into contract coverage** — keep one row per independent ADR obligation. Record the requirement in recognizable ADR language, one of `PROVEN`, `VIOLATED`, `UNVERIFIED`, or `CONTRADICTED`, the ADR basis, how the implementation meets or fails it, exact code or execution evidence, and the tests run. Include the ADR's implementation-independent observable evidence when selecting verification. Never bundle several requirements into one row. `PROVEN` means current evidence supports the obligation and no counterexample was found; it is not a mathematical proof.
