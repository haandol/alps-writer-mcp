...
| R5  | Vertical slice            | `structure.md` "디렉토리 구조" + "흔한 context · subdomain 예시 — 안티패턴 카테고리". (a) 최상위 폴더가 bounded context, leaf(피쳐 sub-folder 또는 단일-피쳐 context)가 피쳐 단위이며 양 세그먼트 모두 기술 레이어 이름(`identity/api` 등)이 아님 (b) Decision 본문이 UI → API → Data 단일 슬라이스 (c) 한 피쳐의 결정이 여러 카테고리로 흩어지지 않음. **subdomainType(core/supporting/generic)·context 그룹핑 메타데이터의 존재/부재는 vertical slice 위반이 아니다** — advisory 라 누락을 fail 로 잡지 않는다                                                                                   |
