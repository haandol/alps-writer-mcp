...
| R5  | Vertical slice            | `structure.md` "디렉토리 구조" + "흔한 카테고리 예시 — 안티패턴 카테고리". (a) 카테고리가 피쳐 단위 (b) Decision 본문이 UI → API → Data 단일 슬라이스 (c) 한 피쳐의 결정이 여러 카테고리로 흩어지지 않음                                                                                                                                                                                  |
...
| R15 | PRD 역참조 없음           | `README.md` "의존성은 단방향, 참조는 어느 방향으로도 직접 적지 않는다" — 본문(Context·Related 포함)에 ALPS 파일 경로(`*.alps.xml`)·`Section N`·feature-id 가 적혀 있으면 fail. PRD 연결은 `.mapping.json` 의 `alpsFeatureId` 에만. PRD 의 user story·acceptance criteria 를 복사해 온 단락도 fail. 위반은 FIX_REQUIRED                                                                    |
...
- `PASS`: 모든 R1–R16 통과, diagram convention 준수, 인덱스/매핑 정합
- `FIX_REQUIRED`: 위반이 있지만 ADR 본문·인덱스·매핑만 손보면 해결됨 — 코드 직독 가능한 항목 제거(R3/R4), 회색지대 보강(R12), Decision Drivers 보강(R13), 대안 추가(R14), dependsOn dangling·self-edge 정정(R16) 도 여기에 해당
