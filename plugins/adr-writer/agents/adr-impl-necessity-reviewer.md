...
- The authoring rules under `docs/adr/` — `authoring-rules.md` and `AGENTS.md` under `docs/adr/` (falling back to the same files under `${CLAUDE_PLUGIN_ROOT}/templates/adr/`). Step 1 needs them to draw the contract line; do not judge that line from memory.
