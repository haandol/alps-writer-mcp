...
- 옵션이 1개뿐 — ADR 가치가 약함 → R12 회색지대 점검에서 fail
...
- [ ] **코드 측 역참조 없음** — 이 ADR 이 다스리는 코드(주석·상수·import)에 ADR ID·경로가 남아 있지 않은가 (코드↔ADR 연결은 코드에도 매핑에도 두지 않는다 — `/adr-sync` 5단계가 grep 으로 점검)
