...
5. Section 7 작성이 끝나면 사용자에게 `/feature-to-adr` 로 일괄 ADR 변환을 시작할지 묻는다 (helper 경로 — adr-writer 플러그인 필요). 한 결정만 직접 작성하고 싶다면 adr-writer 의 `/adr-new <category>` 도 안내한다. 두 경로 모두 adr-writer 플러그인 설치가 전제이며, 미설치 시 `/plugin install adr-writer@alps-writer` 를 권유한다.
