...
6. From the selected starting point:
   - `get_alps_section_guide(N)` → `get_alps_section(N)` → infer a complete draft from available context → ask only about material uncertainty → show a concise plain-text approval digest and confirm → call `save_alps_section(N, ...)` once per approved `X.n` subsection → move to the next section only once confirmed
   ...
   - Treat every guide's questions as an extraction checklist, not an interview script. First use the user's messages and references, approved prior Sections, logical consequences of confirmed contracts, established domain conventions, and dominant reversible MVP defaults.
   - Convert recoverable business variables into concrete, implementation-independent product constants. Generalization means the contract survives a technology change; it does not mean blurring exact values, allowed states, permissions, ordering, units, or success conditions.
   - Ask no question when the available evidence supports one safe draft. Ask one focused question only when multiple valid outcomes remain and the choice changes product value, scope, money, permissions, legal/regulatory/privacy/safety policy, irreversible data meaning, an external promise, acceptance, or learning. Ask at most two only when they cannot be separated.
   - Mark important constants not directly supplied by the user as `AI-inferred` with a short basis in the approval digest. They are proposals, not saved facts, until the user approves them.
...
**Rule**: inference-first drafting is the default, but inferred content is never silently saved. Atomic confirmation is the default. Batch confirmation requires explicit opt-in or a complete structured source and still preserves separate section/Feature drafts and save calls. Never skip an incomplete unit or save without user confirmation.
