...
4. Starting from Section 1, **one section at a time**, in order:
   ...
   - Never skip a section at your own discretion. Even one that looks trivial must be seen and approved by the user before moving on.
5. **Section 7 (Feature-Level Specification)** needs particular care. Confirm and save each Feature subsection (7.1, 7.2, …) **individually, one at a time.** Even when a subsection looks small or similar to the previous Feature, never batch them — go through a separate confirmation step for every 7.x.
6. Once Section 7 is written, ask the user whether to start the bulk ADR conversion with `/feature-to-adr` (the helper path — requires the adr-writer plugin). If they want to write just one decision directly, also point them to adr-writer's `/adr-new <category>`. Both paths presume the adr-writer plugin is installed; if it is not, give the instruction matching the current client — `codex plugin add adr-writer@alps-writer` for Codex, `/plugin install adr-writer@alps-writer` for Claude Code.

**Rule**: never generate several sections at once. Take the user's confirmation per section, and never skip any section without their approval. For Section 7, confirm every Feature subsection (7.x) individually. Never save without user confirmation.
