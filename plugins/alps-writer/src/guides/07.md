...
3. For each key action, what happens end-to-end from UI through API to data store? (Think of it as a vertical slice — one feature cutting through all layers, so it can be built and tested on its own.)
