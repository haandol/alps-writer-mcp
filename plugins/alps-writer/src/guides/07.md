...
- `/feature-to-adr` transfers every implementation-relevant Feature contract into one or several ADRs. At least one real ADR owns each transferred Feature's reproducible contract; independent durable decisions remain separate. After handoff, ADRs are the implementation authority and the PRD is legacy planning context. Explicit re-import compares semantics against the current ADRs; equivalent wording is a no-op and removed contracts are never deleted automatically.
