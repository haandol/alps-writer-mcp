...
<inference_first>
Infer each complete Feature draft from Sections 3 and 6, references, existing contracts, logical
consequences, domain rules, and dominant reversible defaults before asking. Treat the questions
below as a regeneration checklist, not an interview script. Ask only when multiple valid values or
rules materially change user-visible behavior, money, permissions, compliance, data meaning,
acceptance, or the Demo checkpoint. Mark important inferred contracts with their basis in the
approval digest.
</inference_first>
...
draft and a separate `save_alps_section` call. Never merge or skip a Feature. Infer each
complete draft from the approved upstream contract before asking.

1. Read and reuse the confirmed Feature list from Section 6 (F1, F2, F3...) without re-asking
2. Proceed sequentially for each Feature:
   a. Infer the Feature using the regeneration checklist below; ask only about material gaps
...
When the score is 7/10 or higher, propose up to three smaller Feature candidates
...
below 7/10, do not propose a split unless the user asks.
...
5. Derive the values and rules the result must honor — limits, quotas, retention periods, size caps, allowed ranges, states, transitions, mandatory inputs, permissions, visibility, ordering, uniqueness, and units. Reuse explicit upstream contracts first, then logical consequences and established domain rules. A dominant reversible default may be proposed as `AI-inferred` with its basis. Ask only when multiple valid values materially change product policy, money, permission, legal/compliance behavior, irreversible data meaning, an external promise, or acceptance. Record exact requirement values and rules; leave out tuning constants a developer may freely pick.
...
- A Feature scored 7/10 or higher must receive up to three user-behavior split candidates plus the option to keep the original; this never blocks approval or saving
- Batch approval is allowed only when `/alps-init` explicitly selected batch mode
- Do not skip a Feature for being small, similar, or inferable — infer its draft and keep every 7.x as its own confirmation step
...
- Never silently invent a requirement value. Reuse evidence or propose a dominant reversible default
  as `AI-inferred` with its basis and obtain approval. If no safe default exists, ask the targeted
  product question. Values a developer may freely choose remain tuning constants and stay out.
</important>
