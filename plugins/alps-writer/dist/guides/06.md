...
- Performance: If a launch scale was already established in Section 4, restate it ("You mentioned ~[N] users at launch") and ask only to pin the CONCURRENT peak, making the total-vs-concurrent distinction explicit. Otherwise ask: "How many people do you expect to use the product at the same time? (Under 10 / Tens / Hundreds / Thousands+)"
...
- Maximum 3 non-functional requirements for MVP — the cap is deliberate: an MVP can only credibly commit to a few measurable guarantees, so this forces picking the few that actually gate release. If a 4th feels essential, merge or drop a weaker one, or defer it via Section 9.
