...
// when a release bumped the six manifests but not the server. Both times the
// server advertised a stale version to every MCP client for multiple releases
// with a green test suite. These tests make that drift a build failure.
...
// The version this plugin's own package.json declares is the reference: it is
// what `pnpm --filter alps-writer` operates on and what the bundle ships beside.
const EXPECTED = readJson("plugins/alps-writer/package.json").version as string;
