...
const ENFORCE = (process.env.ALPS_ADR_ENFORCE || "warn").toLowerCase();
...
function summarizeMapping(mapping, cwd) {
  ...
  if (cats.length === 0) {
    return "(empty — no ADRs registered yet; new categories must be created via /feature-to-adr or /adr-cycle)";
  }
  const lines = [];
  for (const [cat, entry] of cats) {
    ...
    if (entry.codePaths?.length) {
      lines.push(`    codePaths: ${entry.codePaths.join(", ")}`);
    }
  }
  return lines.join("\n");
}
...
function main() {
  ...
  const directive = [
    ...
    "1. 아래 매핑 스냅샷에서 영향 받는 카테고리를 찾아 docs/adr/<category>/ 의 ADR을 먼저 읽는다. 신규 영역이면 /feature-to-adr (또는 /adr-cycle) 로 새 카테고리/번호를 정한다.",
    ...
    "",
    `PreToolUse 훅이 매핑 누락·stale ADR·미커버 source 영역 편집을 감지해 경고한다 (mode: ${ENFORCE}). 그 경고가 떴을 때 그대로 진행하지 말고 사이클 1단계로 돌아가라.`,
    "",
