...
- **Risk-based implementation review** — every review derives the complete implementation scope from all ADR decisions and contract rows, tracing direct and indirect call paths plus related tests instead of treating the diff as a ceiling. Localized implementations use a sufficiency perspective and targeted tests; protected-surface or broad implementations add separately grounded necessity/sufficiency perspectives. Both modes generate and open the same validated standalone HTML Evidence Package. The report uses the user's language when established, otherwise the target ADR's language; it states ADR intent first, follows the most important verified user, operator, or system flow, then shows findings before the collapsible evidence appendix. Its free-response comprehension check stays collapsed and can reveal criteria only after answer entry; the ordinary main-session completion response never prints Q1 or starts grading, and semantic grading begins only when the user explicitly requests it
