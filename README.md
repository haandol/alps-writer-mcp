...
- **Risk-based implementation review** — localized changes use a decision ledger, concise read-only implementation choices, an isolated sufficiency pass, and targeted tests; protected-surface changes add independent necessity/sufficiency reviews, with diagrams and repair guidance only when the evidence needs them. Intent and regeneration completeness are approved before implementation, so completion review does not repeat a routine human gate
