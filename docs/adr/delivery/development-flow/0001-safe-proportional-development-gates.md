...
7. **7점** — 이해 비용이 높다. Section 7 Feature에는 분할 후보를 제시한다.
...
`4~6점`은 권장 범위다. `1~3점`은 낮은 부하, `7~8점`은 높은 부하, `9~10점`은 매우 높은 부하다. 낮은 점수는 자동 병합 근거가 아니며 높은 점수는 승인·저장·구현을 차단하지 않는다.

Section 7 Feature의 인지비용이 **7점 이상이면** 승인 전에 최대 세 개의 더 작은 Feature 후보를 자동 제안한다. 제안은 현재 Feature를 대체하거나 승인을 막지 않는다. 사용자는 후보를 선택하거나 원래 Feature를 그대로 유지할 수 있다.
...
ADR은 점수만으로 자동 분할을 제안하지 않는다. 사용자가 분할을 요청하고 여러 독립 결정이 섞였을 때만 최대 세 개의 ADR 후보를 제안한다.
...
승인된 ADR revision이 바뀌지 않았다면 `/adr-impl`은 구현 계획을 범위, 구현 단위, 테스트, 인지비용과 중요한 전제를 포함한 **비차단 진행 상황**으로 공유하고 응답을 기다리지 않고 진행한다. 계획 중 새 계약이나 결정, 모순된 전제, 틀릴 경우 계약·안전을 훼손할 수 있으나 검증되지 않은 외부 전제, 승인 범위 밖의 파괴적·광범위 변경을 발견한 경우에만 사용자 판단을 요청한다.
...
- 계획 중 새 계약·결정, 모순, 계약·안전에 영향을 주는 미검증 전제 또는 승인 범위 밖의 파괴적·광범위 변경을 발견한 경우에만 구현 전에 사용자 판단을 요청한다.
...
- 1~3점은 자동 병합 근거가 아니고 7~10점은 승인, 저장이나 구현을 차단하는 근거가 아니다.
- 인지비용의 기본 출력은 `인지비용: N/10` 한 줄이며 점수 근거나 축별 계산을 함께 출력하지 않는다.
- Section 7 Feature의 인지비용이 7점 이상이면 승인 전에 최대 세 개의 의미 있는 Feature 분할 후보를 자동 제시한다.
...
- Section 7 Feature의 인지비용이 7점 미만이면 사용자가 요청하지 않는 한 분할 후보를 제시하지 않는다.
- ADR은 인지비용 점수만으로 자동 분할을 제안하지 않고 사용자가 요청한 경우에만 최대 세 개의 의미 단위 후보를 제시한다.
...
- 사용자는 Section 7 Feature의 예상 이해 비용을 확인하고 7점 이상이면 바로 분할 후보를 비교할 수 있다.
...
- 7점 경계에서 불필요한 Feature 분할 제안이 나타날 수 있다.
