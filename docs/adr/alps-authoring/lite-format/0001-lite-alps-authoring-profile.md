...
자유 형식의 짧은 PRD는 작성은 빠르지만 사용자 행동, 상태, 권한, 제한, 실패 보장과 관찰
가능한 완료 결과를 일관되게 남기지 못한다. Lite 문서도 ALPS의 AI 주도 작성, 명시적 승인,
범위 명시와 재현 가능한 제품 요구사항 원칙을 유지해야 한다.

## Decision Drivers

- 기술 지식이 없는 기획자와 PM이 사용하는 제품 기획 용어로 작성해야 한다.
- 완성된 문서만 읽고 핵심 목업과 PoC 흐름, 화면 상태와 성공 조건을 재현할 수 있어야 한다.
- 구현 기술과 아키텍처를 요구하지 않으면서 제품 동작을 바꾸는 값과 규칙은 보존해야 한다.
- 기존 ALPS 문서와 작성 흐름은 변경 없이 계속 동작해야 한다.
- Lite 작성도 저장 전 승인, 중단 후 재개와 부분 완료 상태를 지원해야 한다.
...
Section 2는 MVP에 포함되는 기능을 `F1`, `F2` 형식으로 식별한다. Section 4는 각 기능을
사용자 목적, 시작 조건, 사용자 행동과 제품 반응, 필수 정보와 적용 기준, 상태와 예외,
관찰 가능한 완료 결과로 구체화한다. Section 5의 화면은 Section 3의 시나리오와 Section 4의
기능에서 도출하고 같은 기능 식별자를 사용한다.

Lite ALPS는 목업과 PoC 작성 입력이다. 구현 준비가 완료된 Full ALPS로 간주하지 않으며
자동으로 ADR 소유권 handoff를 시작하지 않는다.
...
- 모든 저장은 요구사항 값, 권한, 상태, 제한, 범위, 실패 보장과 성공 조건을 포함하는
  plain-text approval digest의 명시적 승인 뒤에 수행한다.
...
- Section 4의 각 Feature는 사용자 목적, 시작 조건, 사용자 행동과 제품 반응, 필수 정보와
  적용 기준, 상태와 예외, 관찰 가능한 완료 결과를 포함한다.
...
- Section 8은 가정과 미해결 사항을 확정된 제품 요구사항과 구분한다.
...
| 가정과 확정 내용을 구분한다.      | Open Questions의 가정과 미해결 사항이 기능의 완료 조건이나 확정 원칙으로 표시되지 않는다. |
| 재개할 수 있다.                   | 작성 중인 Lite 문서를 다시 로드하면 완료 상태가 유지되고 첫 미완료 단위를 안내한다.       |
| 실패 시 원본을 보존한다.          | 유효하지 않은 저장이나 다른 프로필의 템플릿 사용 뒤에도 파일 내용이 요청 전과 같다.       |
| 목업·PoC 입력을 재현한다.         | 내보낸 문서만으로 대표 시나리오, 주요 기능, 화면, 상태와 PoC 성공 판정을 확인할 수 있다.  |
...
- Lite 문서도 범위, 상태, 권한, 실패와 성공 조건을 명시적으로 보존한다.
