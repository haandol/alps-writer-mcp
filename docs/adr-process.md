...
- **A fresh draft is verified once, not reviewed twice.** `/adr-new` authors under the same rules `adr-reviewer` applies (R1-R20), so it runs the deterministic harness and then its own pass over the judgment rules, rather than spawning a reviewer whose punch list would mostly restate what the author just did right. `/adr-review` supplies the independent read where that authoring context is gone — an ADR **edited by hand, changed by another session, or inherited** — and runs on request, not automatically after authoring.
