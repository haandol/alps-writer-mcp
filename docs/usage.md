...
| `/adr-impl-review [category] [--mode standard\|full]` | Find the complete ADR implementation rather than stopping at the diff, run the risk-selected review, explain ADR intent and the most important verified flow, group findings into straightforward action cards, and report the exact path of the common standalone HTML report without opening it automatically. Questions remain collapsed in the report; the ordinary completion response never starts the quiz, and interactive grading is available only by explicit user request                                                 |
