...
5. `/adr-impl-review <category>` derives the complete implementation scope from every ADR decision and contract row, tracing direct and indirect call paths plus related tests; a supplied or discovered diff remains separate change context and never limits sufficiency coverage. It selects `standard` or `full` from the protected surfaces and complete scope. The report uses the user's language when established, otherwise the target ADR's dominant language. It starts with verdict, impact, action, risk, and `ADR intent`, follows subject-specific narrative sections ordered by importance, then shows findings before a collapsible evidence appendix. Both modes validate and generate `adr-impl-review-report.html`, whose table of contents links the narrative, findings, contract evidence, and comprehension check. Supported Mermaid relationships and fenced code render in the page; unsupported Mermaid keeps a source fallback. Proven coverage, scope, metrics, and implementation choices are collapsed, while exceptional coverage is open. The free-response questions remain collapsed and reveal criteria only after answer entry and an explicit self-check; interactive semantic grading still begins only when the user explicitly requests it. `PASS` promotes a `Proposed` ADR to `Accepted` independently of comprehension readiness.
...
| `/adr-impl-review [category] [--mode standard\|full]` | Find the complete ADR implementation rather than stopping at the diff, run the risk-selected review, explain ADR intent and the most important verified flow, show findings before collapsible evidence, and open the common standalone HTML report. Questions remain collapsed in the report; the ordinary completion response never starts the quiz, and interactive grading is available only by explicit user request                                                                                                             |
