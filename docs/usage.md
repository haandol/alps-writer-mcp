...
5. `/adr-impl-review <category>` selects `standard` or `full` from the protected surfaces changed. Its Evidence Package starts with verdict, impact, action, and risk, then follows the predictable `Background → Intuition → Code walkthrough → Comprehension check` spine. Only the headings and order are fixed; paragraphs, lists, examples, tables, and grounded Mermaid remain subject-specific. The last section has one to five free-response questions. `PASS` promotes a `Proposed` ADR to `Accepted`, but does not prove human understanding; do not open or send the PR until every question passes.
...
| `/adr-impl-review [category] [--mode standard\|full]` | Run the risk-selected completion review against the pre-approved ADR baseline; both modes use flexible fixed-order Background/Intuition/Code walkthrough/Comprehension check sections, preserve read-only evidence, and keep PR comprehension readiness separate from the code verdict                                                                                                                                                                                                                                                |
...
Three dependency-free scripts verify the cycle's artifacts without an LLM judgment call. Two cover ADR well-formedness; `adr-impl-review-validate.mjs` requires a non-empty At a glance handoff, fixed explanation/check section order, one to five questions with hidden grading criteria, and requirement-by-requirement `contractCoverage`. It rejects `PASS` unless every row is `PROVEN`, validates notable implementation choices with ADR-intent fit, and checks any evidence-required repair guidance. Full mode may generate an interactive HTML report that shows the questions without exposing their answer criteria.
