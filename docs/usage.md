...
5. `/adr-impl-review <category>` selects `standard` or `full` from the protected surfaces changed. Its Evidence Package starts with verdict, impact, action, and risk for a first-time junior reader, then preserves contract-by-contract evidence. Multi-participant, state, boundary, data, or failure flows receive the smallest grounded Mermaid. `PASS` promotes a `Proposed` ADR to `Accepted`; an unchanged existing `Accepted` ADR remains `Accepted` throughout reinforcement work.
...
| `/adr-impl-review [category] [--mode standard\|full]` | Run the risk-selected completion review against the pre-approved ADR baseline; both modes lead with verdict/impact/action/risk, preserve read-only evidence, and add grounded Mermaid for complex flow, state, boundary, data, or failure relationships                                                                                                                                                      |
...
Three dependency-free scripts verify the cycle's artifacts without an LLM judgment call. Two cover ADR well-formedness; `adr-impl-review-validate.mjs` requires a non-empty At a glance handoff and requirement-by-requirement `contractCoverage`, rejects `PASS` unless every row is `PROVEN`, validates notable implementation choices with ADR-intent fit, and checks the common review report and any evidence-required repair guidance. Full mode may generate an interactive HTML report whose first screen mirrors impact, action, and risk; coverage and implementation choices remain read-only context while findings can receive rulings.
