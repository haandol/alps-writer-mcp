...
| `/adr-impl-review [category] [--mode standard\|full]` | Run the risk-selected completion review; full mode adds necessity review and detailed repair artifacts against the pre-approved ADR baseline                                                                   |
...
Three dependency-free scripts verify the cycle's artifacts without an LLM judgment call. Two cover ADR well-formedness; `adr-impl-review-validate.mjs` validates concise standard-mode artifacts or the full repair guide. Only full mode generates the interactive HTML report. Claude Code uses named reviewer definitions when available; Codex loads the same definitions into generic subagents. On Amazon Bedrock, review skills treat subagents as unavailable, do not retry the known input validation error, continue document/implementation review as main-session passes, and keep refactoring proposal-only.
