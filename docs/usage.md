...
| `/adr-impl-review [category] [--mode standard\|full]` | Run the risk-selected completion review; both modes expose exact AI-selected implementation choices, while full mode adds necessity review, grounded Mermaid repair artifacts, and interactive rulings against the pre-approved ADR baseline |
...
Three dependency-free scripts verify the cycle's artifacts without an LLM judgment call. Two cover ADR well-formedness; `adr-impl-review-validate.mjs` validates the decision ledger, the Implementation Choice Ledger, concise standard-mode artifacts, or the full repair guide. Only full mode generates the interactive HTML report, where findings and AI-selected code-level defaults are reviewed separately. Claude Code uses named reviewer definitions when available; Codex loads the same definitions into generic subagents. On Amazon Bedrock, review skills treat subagents as unavailable, do not retry the known input validation error, continue document/implementation review as main-session passes, and keep refactoring proposal-only.
