...
6. **구현 세부 배제**: `authoring-rules.md` "ADR에 포함하지 않는 것" 표 + `README.md` "코드 직독 테스트" 그대로 적용 — 통합 과정에서 코드를 읽으면 자명한 항목(함수 책임, 필드 타입, 환경 변수, 의사코드 등)이 옛 ADR 들에 섞여 있었다면 통합본에서 제거한다 (회색지대만 남긴다)
